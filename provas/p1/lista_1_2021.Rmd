...
```{r}
c(qt((0.1/2), 80, lower.tail = TRUE), qt(1-((0.1/2)), 80, lower.tail = TRUE))
...
Como a estatística de teste caiu fora da região crítica, rejeita-se H~0~ e admite-se que $\theta$ é diferente de 0 a um nível de significância de 10%.
...
![](images/3a_2.png){width="400"}

![](images/3a_3.png){width="400"}

![](images/3a_4.png){width="400"}

```{=tex}
\justifying
\setlength{\parindent}{0cm}
```
Caso 1: $EQM_{FRP}=EQM_{média} \leftrightarrow \hat\beta_1\hat \sigma_{xy}=0$

Porém,

$$
\hat{\beta}_{1}=\frac{\sum_{i=1}^{N}\left(y_{i}-\bar{y}\right)\left(x_{i}-\bar{x}\right)}{\sum_{i=1}^{N}\left(x_{i}-\bar{x}\right)^{2}} = \frac{\hat\sigma_{xy}}{\hat\sigma^2_{x}}
$$

Logo,

$$
EQM_{FRP}=EQM_{média} \leftrightarrow \hat \sigma_{xy}=0
$$

Caso 2: $EQM_{FRP} < EQM_{média} \leftrightarrow \hat\beta_1\hat \sigma_{xy}> 0$, pois

$$
\begin{aligned}
EQM_{FRP}-EQM_\text{média}&=\cancel{\hat\sigma^2-\hat \sigma^2}-2\hat\beta_1\hat\sigma_{xy} \\
&=-2\hat\beta_1\hat\sigma_{xy} \\
\end{aligned}
$$

Assim, $\hat\beta_1\hat\sigma_{xy}>0 \to EQM_{FRP}<EQM_\text{média}$.

Caso 3: $EQM_{FRP} > EQM_{média} \leftrightarrow \hat\beta_1\hat \sigma_{xy}< 0$

No entanto,

$$
\hat{\beta}_{1} = \frac{\hat\sigma_{xy}}{\hat\sigma^2_{x}}
$$
$$
\hat\beta_1\hat \sigma_{xy}=\frac{\hat\sigma_{xy}}{\hat\sigma^2_{x}} \cdot \hat \sigma_{xy} = \frac{\hat\sigma_{xy}^2}{\hat\sigma^2_{x}}>0
$$

Portanto, $\hat\beta_1\hat \sigma_{xy}<0$ não é possível, o que implica que EQM~FRP~ $\le$ EQM~média~ $\forall (x_i,y_i)$.
