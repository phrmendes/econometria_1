...
2*pt(-1.664125, 80, lower.tail = TRUE)
```
...
![](images/2.png){width="450"}

![](images/3.png){width="450"}

![](images/4.png){width="350"}

\justifying

Logo, $E(\hat\beta_1|X)=\beta_1$, o que justifica a passagem.

### 2) b. {.unnumbered}

$$
Var(\hat \beta_1|X) = E[(\hat \beta_1-\beta_1)^2|X]=(?)=E\left\{\left[\frac{\sum^N_{i=1}u_i(x_i-\bar x)}{\sum^N_{i=1}(x_i-\bar x)^2}\right]^2\right\}
$$
...
-   [ ] *(B) Fórmula do estimador do coeficiente*
...
-   [ ] *(A) Aplicação do quadrado perfeito a um somatório*
...
-   [ ] *(C) Hipótese de homocedasticidade e ausência de autocorrelação*
