...
\setlength{\parindent}{0cm}
```
Caso 1: $EQM_\text{FRP}=EQM_\text{média} \leftrightarrow \hat\beta_1\hat \sigma_{xy}=0$
...
$$
EQM_{FRP}=EQM_\text{média} \leftrightarrow \hat \sigma_{xy}=0
$$

Caso 2: $EQM_\text{FRP} < EQM_\text{média} \leftrightarrow \hat\beta_1\hat \sigma_{xy}> 0$, pois
...
Assim, $\hat\beta_1\hat\sigma_{xy}>0 \to EQM_\text{FRP}<EQM_\text{média}$.

Caso 3: $EQM_\text{FRP} > EQM_\text{média} \leftrightarrow \hat\beta_1\hat \sigma_{xy}< 0$
...
Portanto, $\hat\beta_1\hat \sigma_{xy}$ nunca assume valores menores que 0, o que implica que EQM~FRP~ $\le$ EQM~média~ $\forall (x_i,y_i)$.
