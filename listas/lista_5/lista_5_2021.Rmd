...
\
*Sim, contradiz, pois o coeficiente para poluição atmosférica no teste de significância individual é significativo, com p-valor menor que 5%. Quando tomada em conjunto com variáveis não significativas como `fem_head` e `fertility_`, a variável não contribui para a explicação da variância de `obitos_por_milhao`.
...
table_a <- tibble::tibble(var = c("(Intercept)", "log(1 + epc)")) |> 
  dplyr::group_by(var) |> 
  tidyr::nest() |>
  dplyr::mutate(
    data = purrr::map(
...
eq_b <- geepc ~ 
  log(1 + epc) + log(1 + pibpc) +  log(1 + apc) +
...
2.  *Há alguma variável cuja insignificância estatística é contra-intuitiva?*\
    Sim, `d_reg_sam` não ser significante é contraintuitivo pelo fato do Brasil ser um grande emissor de gases do efeito estufa por conta do desmatamento.
...
df1 <- table_a |> 
  dplyr::mutate(Variável = dplyr::case_when(
    Variável == "log(1 + epc)" ~ "log(1 + epc_a)"
  )) |> 
  dplyr::slice(2)

df2 <- table_b |> 
  dplyr::mutate(Variável = dplyr::case_when(
    Variável == "log(1 + epc)" ~ "log(1 + epc_b)"
  )) |> 
  dplyr::slice(2)
...
vcrit <- round(qchisq(0.05, 2, lower.tail = F), 2) # valor 
```

1.  *Valor observado da estatística do teste:* `r round(lm_test$Chisq[2], 2)`
2.  *Valor crítico ao nível de significância de 5%:* `r vcrit`
3.  *P-valor:* `` r round(lm_test$`Pr(>Chisq)`[2], 2) ``
